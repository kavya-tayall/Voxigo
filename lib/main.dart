...
import 'widgets/login_state_provider.dart';
...
class ParentBasePageState extends State<ParentBasePage> {
  ...
  bool atBasePage = true; // Track if we are at the base page
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      canPop: !atBasePage, // Allow back navigation only if not at the base page
      onPopInvokedWithResult: (didPop, result) async {
        if (!atBasePage) {
          // If not at the base page, allow the pop action
          return Future.value(true);
        } else {
          // If at the base page, reset to index 0 and block the pop action
          setState(() {
            _selectedIndex = 0;
            atBasePage = true;
          });
