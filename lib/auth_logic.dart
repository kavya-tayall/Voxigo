...
class AuthService {
  ...
  // Register Parent with Email Verification
  ...
  Future<void> _fetchAndStoreChildrenData(
      List<dynamic> childrenIds, BuildContext context) async {
    ...
    if (childQuery.docs.isNotEmpty) {
      ...
      try {
        ...
        for (int i = 0; i < data.length; i++) {
          try {
            // Handle the individual download operations safely
            await downloadMp3(data[i]['link']);
          } catch (e) {
            print('Error downloading MP3 for index $i: $e');
          }

          try {
            await downloadCoverImage(data[i]['image']);
          } catch (e) {
            print('Error downloading cover image for index $i: $e');
          }
        }
      } catch (e) {
...
class UserService {
  ...
  Future<DocumentReference?> registerChild(String parentId, String firstName,
    ...
    await uploadJsonFromAssets(
        'assets/board_info/board.json', '/user_folders/$username/board.json');
    await uploadJsonFromAssets(
        'assets/songs/music.json', '/user_folders/$username/music.json');
